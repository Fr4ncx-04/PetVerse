        ...
        >
            ...
            <div className="p-6">
                ...
                <div className="flex items-center text-sm mb-4">
                    <div className={`w-24 h-4 rounded ${pulseColor} mr-4`}></div> 
                    <div className={`w-24 h-4 rounded ${pulseColor}`}></div> 
                </div>

                <div className={`h-6 w-3/4 rounded ${pulseColor} mb-2`}></div>
                {/* Excerpt Placeholder */}
                <div className={`h-4 w-full rounded ${pulseColor} mb-2`}></div>
                <div className={`h-4 w-5/6 rounded ${pulseColor} mb-4`}></div>
                {/* Category and Read More Placeholder */}
                <div className="flex items-center justify-between">
                    <div className={`w-20 h-4 rounded ${pulseColor}`}></div>
                    <div className={`w-24 h-6 rounded-full ${pulseColor}`}></div> 
                </div>
            </div>
        </motion.article>
    );
...
          : 'Consejos Esenciales para el Cuidado de Mascotas',
      excerpt:
        language === 'en'
...
          : 'Entrenando a tu Nuevo Cachorro',
      excerpt:
        language === 'en'
...
  ];
    ...
    const filteredPosts = loading ? [] : blogPosts.filter( 
        (post) =>
...
              type="text"
              placeholder={
                language === 'en' ? 'Search articles...' : 'Buscar artículos...'
              }
              value={searchTerm}
              onChange={handleSearchChange}
              className={`w-full px-4 py-2 pl-10 rounded-lg ${theme === 'dark' ? 'bg-gray-800 border-gray-700 text-white' : 'bg-gray-200 border-gray-300 text-gray-800'}
...
          {loading ? (
              ...
              Array.from({ length: 3 }).map((_, i) => <SkeletonBlogPost key={i} />) // Render 3 skeleton cards
          ) : (
              // Show actual blog posts once loaded and filtered
