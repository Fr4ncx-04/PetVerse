...
    if (!user?.IdUser) {
    ...
    setIsLoading(true);
    axios
      .get(`http://localhost:3000/api/pets/getPetsByUser/${user.IdUser}`)
      .then((res) => {
...
useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        const delayDuration = 2000;
        setTimeout(() => {
            setAdoptionPets(res.data.slice(0, 3)); // Limitar solo a las primeras 3 mascotas
            setIsLoadingAdoption(false); // Desactivar el estado de carga
        }, delayDuration)
        })
      .catch((err) => {
...
              tab === "my-pets"
                ? "bg-green-600 text-white border-green-600" // Active state
